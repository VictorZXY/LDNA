...
from torch_geometric.nn.aggr import AttentionalAggregation, VariancePreservingAggregation
...
class VPA(nn.Module):
    def __init__(self, *,
        ...
        # Variance-Preserving Aggregation replaces the neighbor sum with sum / sqrt(N). Datasets
        # with edge features use the edge-aware GINEConv; edge-less datasets (e.g. MNISTSuperpixels)
        # fall back to the plain GINConv. The branch is driven by whether the resolver wired an
        # `edge_encoder`, so no dataset logic is needed here.
        self.use_edge_attr = edge_encoder is not None
        ...
        for in_channels, out_channels in zip(channel_list[:-1], channel_list[1:]):
            if self.use_edge_attr:
                self.convs.append(GINEConv(
                    ...
                    aggr=VariancePreservingAggregation(), **kwargs))
            else:
